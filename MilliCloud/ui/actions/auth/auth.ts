...
import { apiBaseUrlServer } from "@/lib";
...
export const createNewUser = async (formData: SignUpFormData) => {
  const url = new URL(`${apiBaseUrlServer}/users`);
...
export const getToken = async (formData: SignInFormData) => {
  const url = new URL(`${apiBaseUrlServer}/tokens`);
...
export const getUserByMe = async (accessToken: string) => {
  const url = new URL(`${apiBaseUrlServer}/users/me?include=roles`);
